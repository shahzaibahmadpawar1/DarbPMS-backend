...
export const requireStationDepartmentAccess = (
    ...
    return async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            // Project department users need access across all station types to manage end-to-end workflows.
            // Keep the stationCode normalization above so controllers operate on station_code consistently.
            if (req.user.department === 'project') {
                next();
                return;
            }
